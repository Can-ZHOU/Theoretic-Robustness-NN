...
class ReLUNet(nn.Module):
    def __init__(self, layer_sizes=None, bias=True, dtype=torch.float32,
                 manual_net=None, isSigmoid=False):
        ...
        if manual_net is None:
            if isSigmoid:
                self.net = self.build_network_Sigmoid(layer_sizes)
            else:
    ...
    def build_network_Sigmoid(self, layer_sizes):
        layers = OrderedDict()
        self.fcs = []
        num = 1
        for size_pair in zip(layer_sizes, layer_sizes[1:]):
            ...
            self.fcs.append(layer)
            num = num + 1
            layers[str(num)] = nn.Sigmoid()
            num = num + 1
        del layers[str(num-1)]
        net = nn.Sequential(layers).type(self.dtype)
        return net
