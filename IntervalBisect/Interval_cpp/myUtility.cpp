...
// convert string to long double / helper function
long double toFloat(string x)
{
    return stof(x);
}
...
// print vector<vector<long double>>
...
vector<vector<vector<long double>>> readParametersMulti(string filename) {
    ...
    string line;
    vector<long double> tmp;
    vector<vector<long double>> resultTmp;
...
bool compare_tuple (tuple<mpfi_float_50, int> a, tuple<mpfi_float_50, int> b) {
    ...
    return compareIntervalGreater(tmp_a, tmp_b);
}
...
vector<vector<string>> convert_to_string_2D_array_IA(vector<tuple<vector<mpfi_float_50>, vector<int>>> array) {
    ...
    for(int i=0; i<array.size(); i++) {
        tmp.clear();
        for(int j=0; j<get<0>(array[i]).size(); j++) {
            mpfi_float_50 tmp2 = get<0>(array[i])[j];
            string lower_bound = lower(tmp2).convert_to<string>();
            string upper_bound = upper(tmp2).convert_to<string>();
            string back = "(" + lower_bound + "," + upper_bound + ")";
            tmp.push_back(back);
...
mpfi_float_50 abs_d(mpfi_float_50 x) {
    mpfi_float_50 lower_x = lower(x);
    mpfi_float_50 upper_x = upper(x);
    mpfi_float_50 zero = {0};

    if (compareIntervalGreater(lower_x, zero)) {
        return {1};
    } else if(compareIntervalGreater(zero, upper_x)) {
        return {-1};
    } else {
        return expand(0, 1);
    }
