...
tuple<vector<vector<string>>, string> get_interval_Lipschitz_CPP(string file_index, long double radius, int inputs_size, int hidden_neural_num, int output_size, bool comparedToCLEVER, int maxIterationNum, long double minGap, int max_boxes)
{
    ...
    vector<long double> tmp = {-2.661554487492470855e+01,-1.636761031598184957e+01};
    X0.push_back(tmp);
    vector<vector<long double>> W1;
    tmp = {-3.580402433872222900e-01,5.779012441635131836e-01};
    W1.push_back(tmp);
    tmp = {-3.235487043857574463e-01,5.656679272651672363e-01};
    W1.push_back(tmp);
    vector<vector<long double>> W2;
    tmp = {-4.630059003829956055e-02,6.910411715507507324e-01};
    W2.push_back(tmp);
    tmp = {1.755061298608779907e-01,-6.035143136978149414e-01};
    W2.push_back(tmp);
    vector<long double> b1;
    b1 = {-7.057379931211471558e-02,6.472071409225463867e-01};
    int Pred_X0 = 0;
